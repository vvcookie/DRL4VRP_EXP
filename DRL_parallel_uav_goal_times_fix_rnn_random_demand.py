# -*- coding: utf-8 -*-
# todo 优化目标为“一辆uav路径的长度”网络参数共享 + v2同时决策+ LRup+ FIX last hh的bug
import os
...
print("优化目标为“一辆uav路径的长度”网络参数共享 + v2同时决策+ LRup+ FIX last hh的bug")
...
class DRL4TSP(nn.Module):
    ...
    def forward(self, static, dynamic, decoder_input=None,
        ...
        last_hh_home = [None] * self.depot_num
        for _ in range(max_steps):  # 主循环
            ...
            for car_id in range(self.depot_num):
                ...
                # 修改last hh
                probs, last_hh_home[car_id] = self.pointer(self.static_hidden, dynamic_hidden, decoder_hidden,
                                              last_hh_home[car_id])  # 得到下一个点的（未mask）概率分布和隐状态。
...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, num_city, max_load=10, car_load=0., max_demand=1., seed=None,
        ...
        demands = torch.full(dynamic_shape, self.max_demand) # fixme 修改掉这里。
        # demands=torch.rand(dynamic_shape) * self.max_demand # 新增！：demand 为 [0, self.max_demand)的浮点数。
...
def reward(static, tour_indices):  # 这个是旧的reward，,depot_number参数没用
    ...
    return total_len_tensor.detach() #total_distance
...
class StateCritic(nn.Module):
    ...
    def forward(self, static, dynamic):
        ...
        output = torch.cat((static_hidden, dynamic_hidden), 1)
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "parallel_train_log_uav_goal_times_fix_rnn", '%d' % num_city, format_now)  # ./vrp/numnode/time
    ...
    for epoch in range(20):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            del critic_est
            torch.cuda.empty_cache()  # todo 修改了
            losses.append(sum(actor_losses)/len(actor_losses)) #list(float)# todo 已修改。
...
def run_multi_alg_test(share_depot, args, algorithm):
    ...
    map_size = 1  # fixme 原代码地图大小默认1，不需要指定。而且本来产生坐标的时候就是0-1范围的浮点。
    car_load = 2 * map_size * 1.4  # 测试
    MAX_DEMAND = 0.1  # 测试 # todo 目前是固定值。----------已修改。
...
def test_generalization_uav_change(shared,run_alg_name,_save_dir):
...
def test_generalization_tower_change(share, run_alg_name,_save_dir):
    parser = argparse.ArgumentParser(description='Combinatorial Optimization')
    parser.add_argument('--seed', default=1234, type=int)
    parser.add_argument('--checkpoint', default=None)
    ...
    if share:
            ...
            args.checkpoint = os.path.join("trained_model", "parallel_uav_goal_10w20epoch+finetunn")
            args.checkpoint = os.path.join("trained_model", "parallel_adv2_10w40epoch")

    # run_alg_name = ["Greedy", "RL"]
    print("比较算法：", run_alg_name)
    reward_list_dict = {}

    tower_list = list(range(50, 201, 50))
    ...
    plt.legend()
    dir = os.path.join(_save_dir)# os.path.join("generalization_test_picture_Parallel")
...
if __name__ == '__main__':
    ...
    if share:
        ...
        # args.checkpoint = os.path.join("../", "trained_model", "parallel_uav_goal_10w20epoch") # 服务器用的
        args.checkpoint = os.path.join("trained_model", "parallel_adv2_10w40epoch")
    ...
    # save_dir="generalization_test_Parallel"
    # test_generalization_uav_change(shared=share,  run_alg_name=["RL","Greedy"],_save_dir=save_dir)
    # test_generalization_tower_change(share=share,run_alg_name=["Greedy","RL"],_save_dir=save_dir)
    # 结束计时
    end_time = time.time()
    # 计算并打印执行时间
