# -*- coding: utf-8 -*-
# todo “ADV2”网络参数共享 + v2同时决策+ LRup+ FIX last hh的bug +加入了ID特征给SHARE.Critic 学习率变小看看是否真的不会收敛
import os
...
print("ADV2”网络参数共享 + v2同时决策+ LRup+ FIX last hh的bug +加入了ID特征给SHARE.Critic 学习率变小看看是否真的不会收敛")
...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size, car_load, depot_num,
        ...
        self.id_encoder = Encoder(3, hidden_size)
    ...
    def forward(self, static, dynamic, decoder_input=None,
        ...
        for _ in range(max_steps):  # 主循环
            ...
            for car_id in range(self.depot_num):
                ...
                # decoder_hidden = self.decoder(decoder_input)  # 更新编码当前位置xy静态信息（B,128,1)
                car_id_hidden = torch.full((batch_size, 1, 1), car_id, dtype=torch.float).to(
                    dynamic.device)  # (B,1,1)
                # car_id_hidden = self.id_encoder(car_id_hidden) # （B,128,1) # todo 要不要转onehot?????
                decoder_hidden = self.id_encoder(torch.cat((car_id_hidden, decoder_input), dim=1))
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "parallel_train_log_adv2_times_fix_rnn_ID", '%d' % num_city, format_now)  # ./vrp/numnode/time
...
if __name__ == '__main__':
    ...
    parser.add_argument('--critic_lr', default=1e-4, type=float) # todo
    parser.add_argument('--max_grad_norm', default=2., type=float)
    ...
    #     args.checkpoint = os.path.join("vrpparallel_train_log_adv2_times_fix_rnn_ID", "50","12_31_12_7_25")
