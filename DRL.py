    ...
    def forward(self, static, dynamic, decoder_input=None,
        ...
        for _ in range(max_steps):  # 主循环
            ...
            if self.update_fn is not None:
                ...
                dynamic[:, 0] = car_load[(car_id + 1) % num_car].unsqueeze(1).expand(-1,num_nodes)
...
class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, num_city, max_load=10, car_load=0., max_demand=1., seed=None,
        ...
        # reward_set=run_greedy_VRP(self.static,num_city,num_depots) # todo 训练的时候别调用啊！！！！！！！
        # analysis.reward_greedy=reward_set
        self.car_load = car_load # 这个是回仓库的时候恢复的值。含义上是maxload
...
def reward2_penalty(static, tour_indices, depot_number):
    """
    暂未启用。
    根据 tour_indices 给出的所有城市/节点之间的欧几里得距离.和原始reward不同的是加上了回仓库次数的惩罚
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task +"_train_log", '%d' % num_city, format_now) # ./vrp/numnode/time
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nodes', dest='num_city', default=100, type=int)  #todo 对齐#########
    ...
    # todo：测试泛化性【画出图片吧】。和更多方法比较、混合训练【正在训练……向下兼容的泛化性会更好吗？】
    # todo: 重构代码。和贪心能够一键对齐【感觉为了对比……迟早要统一出一个环境接口和算法接口……】
    # todo:  抢夺决策？？？怎么写实现？
    # todo:  共享仓库【可以先改然后丢去训练】
    # todo:  随机事件（这个太难写了）
    # todo: 【????到底路程和城市数量的关系是什么。……】
    # todo 【就是说要不要重新改“去他人仓库”的代码然后训练。】
