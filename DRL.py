...
if __name__ == '__main__':
    ...
    parser.add_argument('--test', action='store_true', default=False)
    parser.add_argument('--task', default='vrp')
    parser.add_argument('--nodes', dest='num_city', default=250, type=int)  #todo 对齐#########
    # parser.add_argument('--actor_lr', default=5e-4, type=float)
    # parser.add_argument('--critic_lr', default=5e-4, type=float)
    parser.add_argument('--actor_lr', default=1e-4, type=float)  # 学习率，现在在训练第4epoch，我手动改了一下
    parser.add_argument('--critic_lr', default=1e-4, type=float)
    ...
    # todo：测试泛化性测试一下最合适的飞机数量？。和更多方法比较、混合训练【向下兼容的泛化性会更好吗：会】
    #  todo ：试试直接用大数据来从0训练。？？？
    #   【是否存在飞机/city越大，共享算法越不明显？测试强化学习的最佳飞机数量、成比例效果。】
    #   【应该会和训练、以及密度有关系吧……全是机场的话策略就不重要了】
