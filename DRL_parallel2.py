...
# todo 此代码goal是优化总路程。且改进了重新选点的实现方式：使用mask来间接修改点。
...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # todo 记得修改不同的gpu编号
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    for epoch in range(180):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
