# -*- coding: utf-8 -*-
# todo adv2 +网络参数共享 + v2同时决策+ FIX last hh+不解决冲突
import os
...
print("adv2 【测试FIX last hh+不解决冲突 】 LR小=1e-4")
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "parallel_train_log_uav_goal_times_fix_rnn_unconflict", '%d' % num_city, format_now)  # ./vrp/numnode/time
    ...
    for epoch in range(40):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
