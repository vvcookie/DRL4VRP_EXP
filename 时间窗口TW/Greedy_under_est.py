...
def DRL4VRP_Problem(tower_num, uav_num, position,est_upper, est_lower, _share):
    ...
    def get_random_factor():
        return est_upper + np.random.rand() * (est_upper - est_upper) #  20+200 share:28.216352640810392
        # return upper # 20+200 no share:29.416347104914276
    ...
    def run():
        ...
        while not all(visited_tower) or any(empty_depot):  # 如果还有电塔没有访问、是否所有无人机都在仓库
            ...
            if in_depot: # fix√ # 如果在仓库。（这个是留给以后扩展成可以连续访问仓库用的）
                if under_est_one*(dis_go + dis_to_depot + demand) > uav.energy:  # 电量不够：自己的任务已经完成了
                    #if dis_go != math.inf:
                ...
                if  under_est_one*(dis_go + dis_to_depot + demand) > uav.energy:  # 电量不够 回到自己的仓库
                    ...
                    # rand_factor = get_random_factor()
                    uav_order.append((uav_index, uav_order.pop(0)[1] + cost+ tower_need))  # 添加任务时间排序
...
if __name__ == "__main__":
    ...
    if share:
        ...
        print(f"Run {run_times} times. 贪心非 Share Average tour length:  {np.mean(reward_set_independent)}")
