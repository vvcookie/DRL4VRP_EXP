...
if __name__ == '__main__':
    ...
    if share:
        args.checkpoint = os.path.join("TW_random_train_log_share","200(tem)")
        # Average tour length in test set:  12.078526926040649
    else:
        args.checkpoint = os.path.join("TW_random_train_log","200(tem)")
        #Average tour length in test set:  12.59974923324585

    reward_rl = run_RL_exp(upper,lower,share, args) # todo 注意update mask 在run multi test 里面还没有改函数引用。
