# -*- coding: utf-8 -*-
"""Copy of DRL4VRP.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1XiLD228na5A4_srRVrHVG9pitMoi4n5d
...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size, car_load,
                 update_fn=None, mask_fn=None, node_distance=None,num_layers=1,
                 dropout=0.):  ##########################################################
    ...
    def forward(self, static, dynamic, decoder_input=None,
        ...
        all_decoder_input = decoder_input  # 所有无人机的初始位置
        decoder_input = all_decoder_input[:, :, 0].unsqueeze(2)  # 当前无人机的初始位置（后面for循环会更新的） # todo 是哪一维
        ...
        car_load = [torch.full((batch_size,), self.car_load) for i in range(num_car)]#fixme load要改
        # todo =====================================
        ...
        tour_idx=[[torch.tensor(tour_idx[i][0]).unsqueeze(1).to(device)] for i in range(num_car)] #???????
        # fixme=========================================

        #ptr=torch.tensor(tour_idx[0][-1])  fixme ptr 大小B 1。ptr更新！！！更新为下一个无人机的当前位置。
        ptr=tour_idx[0][-1].clone().detach()
        ...
        max_steps = sequence_size if self.mask_fn is None else 2000  # 如果没有设置mask函数，为了避免死循环，这是最大步数。
        #distance = self.node_distance(static)
        ...
        for _ in range(max_steps):  # 主循环
            ...
            if self.update_fn is not None:
                ...
                # 获取最后一个访问的点
                #last_visited = tour_idx[-1] # fixme 改维度了
                ...
                # fixme 新增=========================dynamic中的旧load储存，更新新load


                car_load[car_id]=dynamic[:,0,0].clone() # 随便取第一个仓库的load就好了【？？？？啥意思啊怎么看不懂了】
            ...
            tour_idx[car_id].append(ptr.data.unsqueeze(1))  # T B 1 这个要是旧的ptr……
            #ptr=torch.tensor(tour_idx[(car_id+1)%num_car][-1])  fixme ptr 大小B 1。ptr更新！！！更新为下一个无人机的当前位置。
            ptr=tour_idx[(car_id+1)%num_car][-1].clone().detach()

            # And update the mask so we don't re-visit if we don't need to
            if self.mask_fn is not None:
                #mask = self.mask_fn(dynamic, ptr.data).detach()  # detach是分离出来，但是不需要梯度信息。
                mask = self.mask_fn(dynamic,distance,ptr.data).detach() # fixme 注意这个ptr和dynamic需要是新的,因为需要根据下一个无人机的当前位置，判断下一个点不能去哪。
...
#     def forward(self, static, dynamic, init_depot, decoder_input=None, last_hh=None):##########################################################
#         """
#         Parameters
#         ----------
#         static: Array of size (batch_size, feats, num_cities)
...
#         # todo 在多仓问题里面，初始位置需要指定并且不重复。或者在本类的forward第一次调用的时候，就指定decode_input
#         if decoder_input is None:
#             print("DRL4TSP decoder input is None=============================================================================================!\nself.x0=\n")
#             #print(self.x0)
...
#         # Structures for holding the output sequences
#         tour_idx, tour_logp = [init_depot.unsqueeze(1)], [] # 最终的访问序列。

#         max_steps = sequence_size if self.mask_fn is None else 100 # 如果没有设置mask函数，为了避免死循环，这是最大步数。
...
#                 # 更新动态信息，传递最后一个访问的点
#                 dynamic = self.update_fn(dynamic, ptr.data, last_visited)
...
#             tour_logp.append(logp.unsqueeze(1)) # 每个时间t都要储存：因为为了计算整条路径出现的概率，所以是logp.sum(). T B 1
#             tour_idx.append(ptr.data.unsqueeze(1)) # T B 1

#             decoder_input = torch.gather(static, 2,ptr.view(-1, 1, 1).expand(-1, input_size, 1)).detach() # 更新当前位置信息。
...
import matplotlib
matplotlib.use('Agg')  # 防止尝试使用图形界面，允许在没有图形界面的环境中运行 todo??
import matplotlib.pyplot as plt


class VehicleRoutingDataset(Dataset):
    def __init__(self, num_samples, input_size, max_load=10, car_load=0,max_demand=1, seed=None, num_depots=-1):  # 增加了 num_depots参数 ###
        ...
        locations = torch.rand((num_samples, 2, input_size + self.num_depots))  # 为每个样本生成 num_depots 个仓库的位置
        ...
        dynamic_shape = (num_samples, 1, input_size + self.num_depots)
        ##############################
        #loads = torch.full(dynamic_shape, 5.)#fixme load
    ...
    def node_distance(self,static):
      # 既然要改成只能回到自己的机场，这个之后需要修改。
        depot_positions = static[:, :, :self.num_depots]
        node_positions = static[:, :, self.num_depots:]
        depot_positions_expanded = depot_positions.unsqueeze(2).expand(-1, -1, node_positions.size(2), -1)
    # 计算每个节点到最近仓库的距离
        distances_to_depot = torch.sqrt(torch.sum((node_positions.unsqueeze(3) - depot_positions_expanded) ** 2, dim=1))
    # 取每行最小值作为每个节点到最近仓库的距离
        ...
        min_distances = torch.cat((torch.zeros(min_distances.size(0), self.num_depots).to(static.device), min_distances), dim=1)
    # 计算所有节点之间的距离
        distances_between_node = torch.sqrt(torch.sum((static.unsqueeze(2) - static.unsqueeze(3))**2, dim=1))  # 计算欧式距离
    ...
    def update_mask(self,dynamic,distance,chosen_idx=None):
        ...
        # 否则，我们可以选择去任何需求量 <load 且不为0 的地方  并且loads-chosen_distance需要大于0【因为仓库demand=-1】
        ...
        has_no_demand = demands[:, self.num_depots:].sum(1).eq(0).float() # 所有city都没有demand  转1和0

        #print("new_mask2",new_mask)

        #combined = (has_no_load + has_no_demand).gt(0) # combine zero：该样本有的 city有 demand 并且 车load不等于0
        combined = has_no_demand.gt(0)
        #在无demand下，限制小车不要在仓库之间乱跑
        if combined.any():  # nonzero：要么车有load=0 要么所有city无demand
    ...
    def update_dynamic(self, dynamic, distance,chosen_idx, last_visited): #加了参数：访问的前一个点。
        ...
        """更新 (载重量, 需求量) 数据集的值。注意！！这里的chosen idx是网络新预测出来的下一个点idx，last visited是当前位置。但是update mask函数里面的chosen idx是当前位置。"""
        last_visited=last_visited.squeeze()
        # 根据是访问仓库还是城市，以不同方式更新动态元素
        ##############################
        visit = chosen_idx.ge(self.num_depots)  # 访问的是城市还是仓库
        depot = chosen_idx.lt(self.num_depots)
        ...
        distance_matrix = distance[:, :-1, :] #距离矩阵
        # 在小批量中 - 如果我们选择访问一个城市，尽可能满足其需求量
        if visit.any():
            diff_distances = distance_matrix[torch.arange(distance_matrix.size(0)), last_visited, chosen_idx.squeeze()].unsqueeze(1)
            # 上一次选择的节点与这次选择的节点的差值
        ...
        depot_visited_idx = last_visited.lt(self.num_depots).to(dynamic.device)
        ...
        all_demands[depot_visited_idx.to(dynamic.device), last_visited.to(dynamic.device)[depot_visited_idx]] = -1. #+ new_load[depot_visited_idx].reshape(-1,2)
        ...
        if depot.any():
          ...
          depot_indices = chosen_idx[depot.squeeze()]
        ...
        return tensor.clone().detach().to(device=dynamic.device)
...
#             all_lines.append(ax.plot([], [], color=colors[j % 10])[0])
#             #ax.legend(loc="upper right", fontsize=5, framealpha=0.5)

#         ax.scatter(static[i,0,num_depots:], static[i,1,num_depots:], s=10, c='r', zorder=2)
#         ax.scatter(static[i,0,:num_depots], static[i,1,:num_depots], s=50, c='k', marker='*', zorder=3)
...
def validate(data_loader, actor, reward_fn, render_fn=None, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        if render_fn is not None and batch_idx < num_plot:
            ...
            print("tour indices size:", tour_indices[0].size())
...
def train_vrp(args):
    ...
    LOAD_DICT = {10: 20, 20: 30, 30: 35, 50: 40, 100: 50, 200:80}
    MAX_DEMAND = 1
    STATIC_SIZE = 2 # (x, y)
    DYNAMIC_SIZE = 2 # (load, demand)

    max_load = LOAD_DICT[args.num_nodes]
    car_load=30.   #########################这里修改load
