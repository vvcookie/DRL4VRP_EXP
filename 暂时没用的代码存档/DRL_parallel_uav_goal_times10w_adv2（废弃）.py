# -*- coding: utf-8 -*-
# todo 优化目标“一辆uav路径的长度+全局长度  加权！”（共享actor）
...
print("优化目标“一辆uav路径的长度+全局长度  加权！”（共享actor）")
...
class Attention(nn.Module):
    ...
    def forward(self, static_hidden, dynamic_hidden, decoder_hidden):
        ...
        hidden = torch.cat((static_hidden, dynamic_hidden, hidden), 1)  # 拼接，B,3H,L # OOM
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "parallel_train_log_uav_goal_times10w_adv2", '%d' % num_city, format_now)  # ./vrp/numnode/time
    ...
    for epoch in range(20):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
        ...
        for batch_idx, batch in enumerate(train_loader):
            torch.cuda.empty_cache() # todo test有没有用。
            static, dynamic, x0 = batch
            ...
            tour_logp=[tour_logp[i].sum(dim=1) for i in range (depot_num)]
            ...
            advantage_global = (reward.sum(0).detach() - critic_est) # todo ……添加：advantage要加上全局总路程的优化。
            advantage_per_uav=(reward-critic_est / depot_num).detach()# todo 暂时使用critic/depot num进行测试。
            actor_optim.zero_grad()
            actor_losses=[]
            # 新增：对每辆无人机的路径进行训练。
            for uav_id in range(depot_num):
                # ---------------------todo 直接在这个地方实现不用的优势函数。方便调试代码----------------------
                # todo aaaaaaaaaa 这里的advantage_global没有detach……所以也反向传播了……不过可能没影响？
                actor_loss = torch.mean((0.5*advantage_per_uav[uav_id]+0.5*advantage_global)* tour_logp[uav_id])  # tour_logp:(B，路程长度）.之所以要sum是为了求路径出现概率
                actor_loss.backward(retain_graph=True)
                actor_losses.append(actor_loss.detach().item()) # 保存一下,用于绘图

            torch.nn.utils.clip_grad_norm_(actor.parameters(), max_grad_norm)
            actor_optim.step()
            actor_optim.zero_grad() # todo test
            torch.cuda.empty_cache()
            # 真实奖励值和估计奖励值的差，作为优势函数(这里是A2C中的advantage)
            # critic_loss是根据优势函数的平方误差计算的


            critic_loss = torch.mean(advantage_global ** 2)
            ...
            actor_losses=sum(actor_losses)/len(actor_losses) # 计算出所有actor 的loss的平均值。。
