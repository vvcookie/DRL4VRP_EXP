# -*- coding: utf-8 -*-
# todo adv 为0.5单无人机路程+0.5总路程 ” actor网络参数【独立】版本 + v2的同时决策(10w 30epo）。
import os
...
print("测试【 adv 为0.5单无人机路程+0.5总路程 actor网络参数【独立】”】数据10w 30epo")
...
class RL_UAV(nn.Module):
    ...
    def forward(self,static,dynamic,decoder_input,last_hh):
        # todo —————————测试！！static_hidden如果每个无人机都是独立的话？？！！
        #if self.static_hidden is None: #?????不对啊我只想要一个图计算一次……
        static_hidden = self.static_encoder(static)  # 只需要计算一次 # 算了只是一次一维卷积，重复就重复吧
...
class DRL4TSP(nn.Module):
    ...
    def __init__(self, static_size, dynamic_size, hidden_size, car_load, depot_num,
                 ...
                 dropout=0.):  ##########################################################
        super(DRL4TSP, self).__init__() # 这是调用父类的init函数进行初始化。
        ...
        # Define the encoder & decoder models [可训练的模块] todo √把这些封装起来，作为真正的actor，名字uav，用于训练。
        # self.static_encoder = Encoder(static_size, hidden_size)  # 静态信息embed编码器
        # self.dynamic_encoder = Encoder(dynamic_size, hidden_size)  # 动态信息embed编码器
        # self.decoder = Encoder(static_size, hidden_size)  # 当前位置信息编码器（为什么叫做decoder
        # self.pointer = Pointer(hidden_size, num_layers, dropout)  # 指针网络（含GRU）
    ...
    def forward(self, static, dynamic, uav_set, decoder_input=None,
                last_hh=None):
        """
        Parameters
        ----------
        ...
        for _ in range(max_steps):  # 主循环
            ...
            for car_id in range(self.depot_num):
                ...
                probs, last_hh = uav_set[car_id](static,dynamic,decoder_input,last_hh)
...
def validate(data_loader,uav_set, actor, reward_fn, render_fn=None, save_dir='.',
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        with torch.no_grad():  # 把requires_grad设置为False,避免反向传播时自动求导，节约了显存
            ...
            tour_indices, _ = actor.forward(static, dynamic,uav_set, x0)
        ...
        reward = reward.sum(0) # todo 已修改reward fn，新增了这行方便下面代码实现保持一致（valid不用区分无人机）
...
def train(uav_set, actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "parallel_train_log_uav_goal_times_ind_net", '%d' % num_city, format_now)  # ./vrp/numnode/time
    ...
    # todo√ 修改：定义AC优化器列表（每个网络一个)
    # 定义AC优化器
    # actor_optim = optim.Adam(actor.parameters(), lr=actor_lr)
    uav_optim_set=[optim.Adam(uav_set[id].parameters(), lr=actor_lr) for id in range(depot_num)]
    ...
    for epoch in range(30):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
        ...
        # actor.train() # todo √修改了理论上来说也train不了……。都没优化器了】
        for idx in range(depot_num):
            uav_set[idx].train()
        critic.train()
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            tour_indices, tour_logp = actor(static, dynamic, uav_set,x0)# tour_logp：list[depot_num:tensor(B,T)]

            reward = reward_fn(static, tour_indices) # reward(B,) 修改了reward 函数！现在depot num * B

            # 新增：为了把优化目标改为独立的uav的路径长度，tour_logp_per_uav：list[depot_num:tensor(B,)]
            ...
            advantage = (reward.sum(0).detach() - critic_est)
            advantage_per_uav=(reward- critic_est / depot_num).detach()# todo test 暂时使用critic/depot num进行测试。

            # actor_optim.zero_grad() # todo √修改清零的优化器。
            for idx in range(depot_num):
                uav_optim_set[idx].zero_grad()

            actor_losses=[]
            # 新增：对每辆无人机的路径进行训练。
            for uav_id in range(depot_num):
                # 1,uav goal
                # actor_loss = torch.mean(advantage_per_uav[uav_id] * tour_logp_per_uav[uav_id])  # tour_logp:(B，路程长度）.之所以要sum是为了求路径出现概率
                # 2,global goal
                # actor_loss = torch.mean(advantage.detach()*tour_logp_per_uav[uav_id]) # 还没试过.真的能这样吗？？？？。应该不行吧/
                # adv2: 对半开
                actor_loss = torch.mean((0.5 * advantage_per_uav[uav_id]+0.5* advantage.detach()) * tour_logp_per_uav[uav_id])

                actor_loss.backward(retain_graph=True)
                actor_losses.append(actor_loss.item()) # 保存一下,用于绘图

            # torch.nn.utils.clip_grad_norm_(actor.parameters(), max_grad_norm) # todo √已修改
            for uav in uav_set:
                torch.nn.utils.clip_grad_norm_(uav.parameters(), max_grad_norm)

            # actor_optim.step() # todo √修改了。
            for idx in range(depot_num):
                uav_optim_set[idx].step()
            ...
            rewards.append(torch.mean(reward.detach().sum(0)).item())
        ...
        for idx in range(depot_num):
            actor_save_path = os.path.join(epoch_dir, f'actor{idx}.pt')  # ./vrp_numnode_time//checkpoints/0/actor.pt
            torch.save(uav_set[idx].state_dict(), actor_save_path)

        # actor_save_path = os.path.join(epoch_dir, 'actor.pt')  #./vrp_numnode_time//checkpoints/0/actor.pt
        # torch.save(actor.state_dict(), actor_save_path) # todo √已修改保存的文件。
        ...
        # 每一个epoch validate一次。
        mean_valid, _ = validate(valid_loader,uav_set, actor, reward_fn, render_fn,
        ...
        if mean_valid < best_reward:  # reward 就是路程总长度。暂定无惩罚项(好像确实不需要额外的乘法）
            ...
            for idx in range(depot_num):
                actor_save_path = os.path.join(save_dir, f'actor{idx}.pt')
                torch.save(uav_set[idx].state_dict(), actor_save_path)

            # actor_save_path = os.path.join(save_dir, 'actor.pt')
            # torch.save(actor.state_dict(), actor_save_path) # todo √已修改
...
def run_RL_exp(share_depot, args):
    ...
    # actor_set=[0]* args.depot_num # 新增的acotr集合，用于存放每个无人机单独的actor网络。

    uav_set=[RL_UAV(STATIC_SIZE,DYNAMIC_SIZE,args.hidden_size,args.num_layers,args.dropout).to(device)
             for _ in range(args.depot_num)]
    ...
    if args.checkpoint:  # 读取之前保存的模型。
        print(f"args.checkpoint:已经有ckpt,读取文件夹:{args.checkpoint}")
        # path = os.path.join(args.checkpoint, 'actor.pt')
        # actor.load_state_dict(torch.load(path, device))  # 加载模型参数 todo 已修改。
        for idx in range(args.depot_num):
            path = os.path.join(args.checkpoint, f'actor{idx}.pt')
            uav_set[idx].load_state_dict(torch.load(path, device))  # 加载模型参数
    ...
    if not args.test:
        ...
        train(uav_set,actor, critic, **kwargs)  # 训练！!!!! todo √ 修改
        print("训练结束。")
    ...
    # todo ??????????????validate 内部还没写！！！！！.
    test_out, test_reward = validate(test_loader,uav_set, actor, reward, render, test_dir, num_plot=1,
...
def run_multi_alg_test(share_depot, args, algorithm):
    ...
    if "RL" in algorithm:
        ...
        # path = os.path.join(args.checkpoint, 'actor.pt') # todo √已修改
        for idx in range(args.depot_num):
            path = os.path.join(args.checkpoint, f'actor{idx}.pt')
            uav_set[idx].load_state_dict(torch.load(path, device))  # 加载模型参数
        ...
        test_loader = DataLoader(test_data, args.batch_size, False, num_workers=0)
        # todo 好吧？？？？？？？？？？？？？？？？？？？？还没写文件读取，这里暂时会报错。。。。。。。。

        RL_test_out, RL_test_reward = validate(test_loader,uav_set, actor, reward, render, test_dir, num_plot=5,
                                         depot_number=args.depot_num)
