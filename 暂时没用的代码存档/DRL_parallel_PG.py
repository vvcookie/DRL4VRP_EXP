# -*- coding: utf-8 -*-
# todo 此代码用于测试去掉critic只保留Actor的纯PG效果。（此时仍然是同时决策，若有必要可以尝试轮流决策
#  完全不行！！收敛不了一点。
    ...
    def forward(self, static, dynamic, decoder_input=None,
        ...
        for _ in range(max_steps):  # 主循环
            ...
            for car_id in range(self.depot_num):
                ...
                decoder_hidden = self.decoder(decoder_input)  # 更新编码当前位置xy静态信息 todo 等debug完之后尝试使用“所有无人机的xy”
...
def train(actor, critic, task, num_city, train_data, valid_data, reward_fn,
    ...
    save_dir = os.path.join(current_dir, task + "_parallel_PG_train_log", '%d' % num_city, format_now)  # ./vrp/numnode/time
    ...
    for epoch in range(20):  # 执行20轮训练 fixme!!!!!!!!能不能把这个写到外面。
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            reward = 0-reward_fn(static, tour_indices) # (B,)

            # Query the critic for an estimate of the reward(向评论家询问奖励的估计值)a
            # critic_est = critic(static, dynamic).view(-1)  # (B,)# fixme
            # 真实奖励值和估计奖励值的差，作为优势函数(这里是A2C中的advantage)
            # advantage = (reward - critic_est) # (B,)# fixme
            ...
            actor_loss = torch.mean(reward.detach() * tour_logp.sum(dim=1))  # 这里仅仅使用reward
            # critic_loss是根据优势函数的平方误差计算的
            # critic_loss = torch.mean(advantage ** 2)# fixme
            # 0梯度反向传播
            ...
            #critic_optim.zero_grad()# fixme
            # critic_loss.backward()# fixme
            torch.nn.utils.clip_grad_norm_(critic.parameters(), max_grad_norm)
            critic_optim.step()
            # 将奖励估计值，真实奖励值，actor损失平均求和后写入空列表中
            # critic_rewards.append(torch.mean(critic_est.detach()).item()) # fixme
...
if __name__ == '__main__':
    ...
    # todo 怎么Valid reward 92.5610, 这么大：因为忘记把“处理冲突”的代码取消注释了
    print("---------这是同时决策版本+仅保留Actor py---------")
    #
    # todo 关于bug：
    #  注意last hh是否有问题：目前会用到上一个无人机……【是不是应该把没量无人机的last hh存起来？但是不解决冲突版本也没有这样】
